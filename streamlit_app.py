...
import os, math, json, warnings, uuid, logging, time, io, tempfile, gc
import numpy as np
import pandas as pd
...
# --- SECTION 4: MethAIne LIBRARY (EXACT LOGIC) ---
...
def preprocess_city(path_or_buffer, city_name=None, seasons=["D-J-F","J-J-A"]):
    if city_name is None:
        city_name = getattr(path_or_buffer, 'name', "Uploaded_City").replace(".csv","")
...
def model_lin_modern(Xtr, ytr, Xte):
    ...
    model = LinearRegression().fit(Xtr[m_mask].reshape(-1, 1), ytr[m_mask])
...
def _model_gp_base(Xtr, ytr, Xte):
    try:
        kernel = C(1.0) * RationalQuadratic(length_scale=20.0) + WhiteKernel(noise_level=CONFIG['OBS_ERR_STD']**2)
        gp = GaussianProcessRegressor(kernel=kernel, normalize_y=True).fit(Xtr.reshape(-1, 1), ytr)
...
def fit_ar_garch_and_simulate(resid, n_sim, use_garch=True):
    try:
        ...
        if use_garch and len(resid) > 30:
            ...
            return ar.simulate(n_sim) + np.random.normal(0, sig, n_sim)
        return ar.simulate(n_sim)
    except: return np.random.normal(0, np.std(resid), n_sim)
...
def bootstrap_iteration(idx, years, t_obs, run_config, model_funcs, td_weights, q, resid, reg_adj):
    ...
    sim_n = fit_ar_garch_and_simulate(resid, len(run_config['future_years']), run_config['USE_GARCH'])
    f_temps = stacked + CONFIG['FUTURE_NOISE_DAMPING'] * sim_n + reg_adj
    return f_temps, np.polyfit(run_config['future_years'] - run_config['future_years'][0], f_temps, 1)[0] * 10.0

# --- SECTION 5: CACHED ANALYSIS ENGINE ---

@st.cache_data(show_spinner=False)
def run_full_analysis(all_city_data, run_config, CONFIG):
    all_results = []
    for city_df in all_city_data:
        ...
        for season, g in city_df.groupby("SEASON"):
            ...
            # Parallel execution (Limited workers to prevent crash)
            n_workers = 2 if CONFIG['FAST_MODE'] else 4
            res = Parallel(n_jobs=n_workers)(delayed(bootstrap_iteration)(i, yrs, t_obs, run_config, m_funcs, td_weights, q, resid, reg_adj) for i in range(run_config['N_BOOT']))
            ...
            gc.collect() # Clear memory after each season
    return all_results
...
def generate_interactive_plot(all_results, eid):
    fig = go.Figure()
    sc = {"D-J-F": ["rgba(93, 173, 226, 1)", "rgba(33, 97, 140, 1)"], "J-J-A": ["rgba(230, 126, 34, 1)", "rgba(135, 54, 0, 1)"]}
    ...
    for d in all_results:
        ...
        fig.add_trace(go.Scatter(x=np.concatenate([p['future_years'], p['future_years'][::-1]]), y=np.concatenate([p['ci_upper'], p['ci_lower'][::-1]]), fill='toself', fillcolor=base_clr.replace('1)', '0.15)'), line=dict(color='rgba(255,255,255,0)'), showlegend=False))
...
def main():
    ...
    if st.button("🚀 Run MethAIne Analysis"):
        ...
        if uploaded_files:
            ...
            st.info("Running demonstration mode...")
        ...
        with st.spinner("Processing climate models... This may take several minutes."):
            all_results = run_full_analysis(all_city_data, run_config, CONFIG)
        ...
        static_fig = generate_static_plot(all_results, eid)
        st.pyplot(static_fig)
        plt.close(static_fig) # Critical for memory

        summary_df = pd.DataFrame([r['summary'] for r in all_results])
        st.dataframe(summary_df)
        
        st.download_button("📥 Download CSV Results", summary_df.to_csv(index=False), f"{run_name}_out.csv", "text/csv")
