...
# --- SECTION 4: THE MethAIne LIBRARY (EXACT LOGIC) ---
...
def run_projection_for_city(city_df, run_config):
    ...
    for season, g_full in city_df.groupby("SEASON"):
        ...
        if CONFIG['PARALLEL_BACKEND'] == 'dask':
            ...
            parallel_results = Parallel(n_jobs=-1)(delayed(bootstrap_iteration)(i, years, temps_obs_err, run_config, model_funcs, weights_time_dependent, q, resid, regional_adjustment) for i in range(run_config['N_BOOT']))
    ...
    if st.button("🚀 Run MethAIne Analysis"):
        with st.spinner("Processing climate models..."):
            ...
            if all_city_data:
                ...
                CONFIG['HIST_FULL_END'] = max_year
                CONFIG['MODERN_END'] = max_year
                CONFIG['FUT_START'] = max_year + 1

            st.text(generate_data_quality_report(all_city_data))

            all_results = []
            for city_df in all_city_data:
                city_results = run_projection_for_city(city_df, {**CONFIG, **run_config})
