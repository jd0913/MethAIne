...
import os, math, json, warnings, uuid, logging, time, io
import numpy as np
import pandas as pd
...
# --- SIDEBAR UI ---
st.sidebar.title("MethAIne 3.1 Settings")
run_name = st.sidebar.text_input("Run Name", "default")
fast_mode = st.sidebar.checkbox("Fast Mode (1-2 mins)", value=True)
parallel_backend = st.sidebar.selectbox("Parallel Backend", ["joblib", "dask"])
...
def get_run_config(config_dict):
    if config_dict["FAST_MODE"]:
        ...
        logging.info("--- RUNNING IN FULL ROBUST MODE ---")
...
def get_embedded_sample_data():
    logging.info("Loading embedded sample data for 'Demo City'.")
    years = np.arange(1940, _HIST_END_YEAR + 1)
...
def preprocess_city(path_or_buffer, city_name=None, seasons=["D-J-F","J-J-A"]):
    if isinstance(path_or_buffer, str) and city_name is None:
        city_name = os.path.basename(path_or_buffer).replace(".csv","")
    
    df = pd.read_csv(path_or_buffer)
    df.rename(columns={"DJF":"D-J-F", "JJA":"J-J-A"}, inplace=True)
    df = df[["YEAR"] + seasons].copy().replace([999.9,-999.9], np.nan)
    tt = df.melt(id_vars=["YEAR"], value_vars=seasons, var_name="SEASON", value_name="TEMP")
    tt["CITY"] = city_name
    tt = tt.dropna(subset=["TEMP"])
    return tt

def generate_data_quality_report(all_city_data, CONFIG):
    ...
    for city_df in all_city_data:
        ...
        for season in city_df['SEASON'].unique():
            ...
            if num_years < CONFIG['MIN_YEARS']:
                report_lines.append(f"  - ❌ {season}: Insufficient data ({num_years} years). This season will be skipped.")
                continue
            else:
...
def choose_block_size(resid, CONFIG):
...
def calculate_time_dependent_weights(model_weights, future_years, CONFIG):
...
def get_regional_adjustment_factor(city_name):
    return (len(city_name) - 6) * 0.01

def model_lin_modern(Xtr, ytr, Xte, CONFIG):
    try:
...
def make_stable(unstable_model_func, CONFIG):
    def stable_model_wrapper(Xtr, ytr, Xte):
        try:
            unstable_pred = unstable_model_func(Xtr, ytr, Xte)
            stable_anchor_pred = model_lin_modern(Xtr, ytr, Xte, CONFIG)
            n_future = len(Xte)
            ...
            return model_lin_modern(Xtr, ytr, Xte, CONFIG)
    return stable_model_wrapper

def _model_gp_base(Xtr, ytr, Xte, CONFIG):
    try:
...
def decadal_hindcast_errors(years, temps, model_funcs, CONFIG):
...
def fit_ar_garch_and_simulate(resid, n_sim, use_garch=True):
    ...
    try:
        ...
        if 'arch' in globals() and use_garch:
...
def fit_emcee_linear(years, temps, CONFIG):
...
def bootstrap_iteration(iteration_index, years, temps_obs_err, run_config, model_funcs, weights_time_dependent, q, resid, regional_adjustment, CONFIG):
    xb, yb = block_bootstrap(years, temps_obs_err, choose_block_size(resid, CONFIG))
...
def run_projection_for_city(city_df, run_config, CONFIG):
    city_name = city_df["CITY"].iloc[0]
    ...
    for season, g_full in city_df.groupby("SEASON"):
        ...
        model_funcs = {
            "poly2": make_stable(model_poly2, CONFIG),
            "pchip": make_stable(model_pchip, CONFIG),
            "ridge": make_stable(model_ridge, CONFIG),
        }
        if run_config['USE_GP']:
            model_funcs["gp"] = make_stable(lambda Xtr, ytr, Xte: _model_gp_base(Xtr, ytr, Xte, CONFIG), CONFIG)
        if run_config['USE_PROPHET']:
            model_funcs["prophet"] = make_stable(_model_prophet_base, CONFIG)

        med_err = decadal_hindcast_errors(years, temps_obs_err, model_funcs, CONFIG)
        ...
        weights_time_dependent = calculate_time_dependent_weights(weights, future_years, CONFIG)

        if CONFIG['PARALLEL_BACKEND'] == 'dask':
            lazy_results = [dask.delayed(bootstrap_iteration)(i, years, temps_obs_err, run_config, model_funcs, weights_time_dependent, q, resid, regional_adjustment, CONFIG) for i in range(run_config['N_BOOT'])]
            parallel_results = dask.compute(*lazy_results)
        else:
            parallel_results = Parallel(n_jobs=-1)(delayed(bootstrap_iteration)(i, years, temps_obs_err, run_config, model_funcs, weights_time_dependent, q, resid, regional_adjustment, CONFIG) for i in range(run_config['N_BOOT']))
        ...
        if run_config['USE_EMCEE']:
            emcee_slopes = fit_emcee_linear(years, temps_obs_err, CONFIG)
...
def generate_interactive_plot(all_results, execution_id):
    ...
    for data in all_results:
        p = data["plot_data"]; s = data["summary"]; city_name, season = s["CITY"], s["SEASON"]
    ...
    fig.update_layout(title=f"MethAIne 3.1: Surface Temperature Projections — ID: {execution_id}", xaxis_title="Year", yaxis_title="Temperature (°C)", template="plotly_white", hovermode="x unified", legend=dict(groupclick="togglegroup"))
    return fig
...
def generate_static_plot(all_results, execution_id):
    ...
    for data in all_results:
        p = data["plot_data"]; s = data["summary"]; color = season_colors[s['SEASON']]
        ...
        ax.plot(np.arange(p['years_full'].min(), p['years_full'].max() + 1), poly_hist_full(np.arange(p['years_full'].min(), p['years_full'].max() + 1)), color=color, linewidth=1.5)
    ...
    ax.set_xlabel("Year"); ax.set_ylabel("Temperature (°C)"); ax.grid(alpha=0.4)
    ax.set_title(f"MethAIne 3.1: Climate Projections — ID: {execution_id}")
    return fig

def generate_pdf_report(all_results, execution_id, output_path):
    doc = SimpleDocTemplate(output_path)
    styles = getSampleStyleSheet()
    story = []
    story.append(Paragraph(f"MethAIne 3.1 Report - ID: {execution_id}", styles['h1']))
    doc.build(story)
...
def main():
    ...
    # Dynamic Config
    CONFIG = {
        "FAST_MODE": fast_mode, "PARALLEL_BACKEND": parallel_backend,
        "N_BOOT_FAST": 100, "N_BOOT_FULL": n_boot_full, "RNG_SEED": 42,
        "OBS_ERR_STD": 0.15, "FUTURE_NOISE_DAMPING": 0.6, "BLOCK_SIZE_DEFAULT": 5,
        ...
        "CALIB_START": 1940, "CALIB_END": 1979, "MODERN_START": 1980, "MODERN_END": _HIST_END_YEAR,
        "FUT_START": _FUT_START_YEAR, "FUT_END": 2100, "FROZEN_VALIDATION_YEAR": 2014, "MIN_YEARS": 10,
    }
    ...
    if st.button("Run Analysis"):
        with st.spinner("Processing..."):
            ...
            if uploaded_file:
                all_city_data = [preprocess_city(uploaded_file)]
            else:
                st.warning("No file uploaded. Using demo data.")
                sample_buffer, city_name = get_embedded_sample_data()
                all_city_data = [preprocess_city(sample_buffer, city_name=city_name)]

            # Data Quality Report
            st.text(generate_data_quality_report(all_city_data, CONFIG))

            all_results = []
            for city_df in all_city_data:
                city_results = run_projection_for_city(city_df, {**CONFIG, **run_config}, CONFIG)
            ...
            st.pyplot(generate_static_plot(all_results, EXECUTION_ID))
            
            # CSV Export
            summary_list = [r['summary'] for r in all_results]
            df_out = pd.DataFrame(summary_list)
            st.download_button("Download Results CSV", df_out.to_csv(index=False), "projections.csv", "text/csv")
